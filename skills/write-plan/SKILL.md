...
**Announce at start:** "I'm using the SKILL:write-plan skill to create the implementation plan."

**Context:** If working in an isolated worktree, it should have been created via the `SKILL:isolated-worktree` skill at execution time.
...
2. **If no saved preference,** ask through `SKILL:ask-user`:
   - header: `Execution`
...
**If Subagent:** Use SKILL:subagent-dev — fresh subagent per task + two-stage review.

**If Inline:** Use SKILL:execute-plan — batch execution with checkpoints
