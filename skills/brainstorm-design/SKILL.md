...
9. **Transition to implementation** — invoke SKILL:write-plan to create implementation plan
...
digraph brainstorm {
    ...
    "Invoke SKILL:write-plan" [shape=doublecircle];
    ...
    "User reviews spec?" -> "Invoke SKILL:write-plan" [label="approved"];
}
```

**The terminal state is invoking SKILL:write-plan.** Do NOT invoke frontend-design, mcp-builder, or any other implementation skill. The ONLY skill you invoke after brainstorming is SKILL:write-plan.
...
- When the question has a known set of likely answers, use `SKILL:ask-user` with those answers as options
- For open-ended questions, use `SKILL:ask-user` with 2-3 suggested answers as options — the user can always type their own answer
...
- After presenting each section, use `SKILL:ask-user`:
...
After spec self-review passes, use `SKILL:ask-user`:
- header: `Spec Review`
...
- Invoke SKILL:write-plan to create a detailed implementation plan
- Do NOT invoke any other skill. SKILL:write-plan is the next step.
...
2. **If no saved preference,** offer consent using `SKILL:ask-user` (this MUST be its own message — do not combine with other content):
...
`<SKILL:brainstorm-design>/visual-companion.md`
